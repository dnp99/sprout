...
vi.mock("@/lib/transactions/repository", () => ({ excludeTransactionsFromBudget: vi.fn() }));

import { getSessionUser } from "@/lib/auth/currentUser";
import { excludeTransactionsFromBudget } from "@/lib/transactions/repository";
...
beforeEach(() => {
  ...
  vi.mocked(excludeTransactionsFromBudget).mockResolvedValue(2);
});
...
describe("POST /api/transactions/exclude", () => {
  ...
  it("requires at least one id", async () => {
    ...
    expect(excludeTransactionsFromBudget).not.toHaveBeenCalled();
  });

  it("scopes a bulk exclusion to the signed-in user", async () => {
    const response = await POST(post({ ids: ["t1", "t2"] }));
    expect(response.status).toBe(200);
    expect(excludeTransactionsFromBudget).toHaveBeenCalledWith("u1", ["t1", "t2"]);
