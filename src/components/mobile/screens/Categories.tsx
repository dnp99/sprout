    ...
    <div className="px-4 pb-8 pt-1.5">
      ...
      <div className="mt-4 flex flex-col gap-2.5">
        {view.groups.map((group) => {
          ...
          return (
            <section key={group.id} className="overflow-hidden rounded-[12px] border border-edge">
              <button
                type="button"
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 text-[13px] font-semibold text-ink">
                            <span className="text-[17px]">{row.emoji}</span>
                            <span className="truncate">{row.name}</span>
                          </div>
                          <div className="mt-1 text-[11px] font-medium text-muted">
                            {formatMoney(row.budgetCents)} budget · {formatMoney(row.spentCents)}{" "}
                            spent
                          </div>
                        </div>
                        <div
